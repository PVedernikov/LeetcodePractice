...
public class NeetCode150
{
    ...
    // Идея: храним две кучи (min и max), которые делят весь поток элементов пополам
    // Если элементов четное количество, то обе кучи содержат равное количество элементов
    // Если нечетное, то в левой куче на 1 больше
    public class MedianFinder
    {
        ...
        {
            if (right.Count > 0 && num > right.Peek()) // добавляем либо вправо, либо в лево, в зависимости от значения
            {
            ...
            while (left.Count < right.Count) // Балансируем кучи
            {
                var val = right.Dequeue();
                left.Enqueue(val, -val);
            }

            while (left.Count - right.Count > 1) // Балансируем кучи
            {
                var val = left.Dequeue();
    ...
    //    Для остальных вершин должно быть in-degree = out-degree.
    ...
    //
    // Объяснение: когда мы добавляем вершину в результат, это значит, что мы прошли по всем ребрам, которые ведут из этой вершины.
